export const APP_VERSION='V0.54.0-alpha.1';
...
export class StartupController{
  ...
  bindButtons(){
    ...
    this.overlay.querySelector('[data-boot-clear]')?.addEventListener('click',()=>{
      try{localStorage.removeItem(PHASER_SAVE_KEY)}catch(error){console.warn('Unable to clear Phaser save',error)}
      location.reload();
    });
  }
  setStatus(message){
  ...
  fail(error,context='啟動'){
    ...
    const failureCount=this.failedAssets.length;
    const detail=this.overlay.querySelector('[data-boot-failures]');
    if(detail)detail.textContent=`進度 ${Math.round(this.progress*100)}% · 失敗素材 ${failureCount}`;
