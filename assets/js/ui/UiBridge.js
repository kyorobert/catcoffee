import {ToastManager} from '../systems/ToastManager.js?v=0540a1';
import {StorePanel} from './StorePanel.js?v=0540a1';
...
export class UiBridge{
  constructor(game,saveAdapter,furnitureConfig,{startup=null}={}){
    ...
    this.store=new StorePanel(document.getElementById('storePanel'),furnitureConfig,saveAdapter,type=>this.scene()?.startPlacement(type));
    this.bindEvents();
    this.bindButtons();
  ...
  bindEvents(){
    this.game.events.on('state-changed',state=>{this.renderState(state);if(!this.store.element.classList.contains('hidden'))this.store.render()});
    ...
    this.game.events.on('boot-load-error',failure=>{
      const current=this.game.registry.get('furniture-load-failures')||[];
  ...
  bindButtons(){
    ...
    document.querySelectorAll('[data-care]').forEach(button=>button.addEventListener('click',()=>{this.scene()?.careCat(button.dataset.care);this.closePanel('carePanel')}));
  ...
  renderState(state){
    ...
    document.getElementById('helperBtn').querySelector('span').textContent=`輔助${state.placementHelper?'開':'關'}`;
  ...
  renderCats(state){
    ...
    list.innerHTML=CAT_PROFILES.map(cat=>{
      ...
      return `<article><b>${cat.icon} ${cat.name}</b><span>${cat.personality}</span><small>飽足 ${stats.satiety||0} · 心情 ${stats.mood||0} · 羈絆 ${stats.bond||0}</small></article>`;
    }).join('');
  }
  ...
  showReport(report=null){
    ...
    content.innerHTML=`<div class="report-grid"><b>日期</b><span>Day ${data.day}</span><b>營收</b><span>🪙 ${Number(data.revenue||0).toLocaleString('zh-TW')}</span><b>服務顧客</b><span>${data.served||0}</span><b>人氣增加</b><span>+${data.reputation||0}</span></div>`;
