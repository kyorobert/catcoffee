...
import {getPurchasableFurniture} from '../core/furniture-catalog-selector.js?v=0577a';
...
export class StorePanel {
  constructor(element, furnitureConfig, saveAdapter, onPlace, options = {}) {
    ...
    this.getProjectionMode = options.getProjectionMode || (() => 'iso');
    this.category = '全部';
  ...
  render() {
    ...
    const visualConfig = Object.freeze(Object.fromEntries(
      Object.keys(this.furniture).map(id => [
        id,
        getFurnitureVisualDefinition(id, projectionMode)
      ])
    ));
    const visibleCatalog = getPurchasableFurniture({definitions:this.furniture,visualConfig});
    ...
    const entries = getPurchasableFurniture({definitions:this.furniture,visualConfig,category:this.category});
