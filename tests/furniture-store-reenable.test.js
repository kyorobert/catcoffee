...
for(const id of V0552_REDRAW_FURNITURE_IDS){
  ...
  assert.ok(entry.visual.texturePathByDirection['down-right'].endsWith('.png?v=0577a'),`${id}: old store thumbnail`);
}
assert.ok(entries.every(entry=>!['prototype','retired'].includes(entry.visual.artStatus)));
console.log('Furniture store re-enable passed: 25 redraws restored, 48 catalog entries visible.');
