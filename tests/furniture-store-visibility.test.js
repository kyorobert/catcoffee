...
assert.equal(purchasable.length,48);
...
assert.equal(getDebugFurnitureCatalog({definitions:FURNITURE_CONFIG,visualConfig:FURNITURE_VISUAL_CONFIG}).length,48);
assert.ok(purchasable.some(entry=>entry.id==='pinkTableLong'));
assert.ok(purchasable.some(entry=>entry.id==='pinkTableLongHardCafe'));
...
console.log('Furniture store selector passed: 25 redraws re-enabled, debug catalog complete.');
