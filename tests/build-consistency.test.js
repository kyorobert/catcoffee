...
import {APP_VERSION, BUILD_ID, SAVE_KEY, BuildMismatchError, assertBuildConsistency} from '../assets/js/config/build-info.js?v=0576b';

const html = readFileSync('index.html', 'utf8');
assert.equal(BUILD_ID, '0576b');
assert.equal(SAVE_KEY, 'catCafePhaserV0540');
assert.equal(APP_VERSION, 'V0.57.6-alpha｜正交互動與入口清理版');
assert(html.includes('data-build-id="0576b"'));
assert(html.includes("window.__CAT_CAFE_HTML_BUILD_ID__ = '0576b'"));
assert(html.includes('./assets/vendor/phaser-3.90.0.min.js?v=0576b'));
assert(html.includes('./assets/js/main.js?v=0576b'));
...
for (const file of runtimeFiles) {
  ...
  if (file.endsWith('.js')) {
    for (const match of source.matchAll(/(?:from\s*|import\s*)["'](\.{1,2}\/[^"']+\.js)(\?v=[^"']+)?["']/g)) {
      assert.equal(match[2], '?v=0576b', `${file} has an unversioned or inconsistent import: ${match[0]}`);
    }
  }
}

assert.equal(assertBuildConsistency('0576b'), true);
assert.throws(() => assertBuildConsistency('0550-old'), error => {
  ...
  assert.equal(error.jsBuildId, '0576b');
...
assert(readFileSync('assets/js/config/cat-config.js', 'utf8').includes("CAT_ASSET_VERSION = '0576b'"));
assert(readFileSync('assets/js/config/furniture-visual-config.js', 'utf8').includes("FURNITURE_REDRAW_ASSET_VERSION = '0576b'"));
